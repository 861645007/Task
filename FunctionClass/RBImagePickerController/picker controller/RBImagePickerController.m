...
-(void)onCancel:(id)sender{
    if([self.delegate respondsToSelector:@selector(imagePickerControllerDoneCancel:)])
        [self.delegate imagePickerControllerDoneCancel:self];
   else
...
-(void)assignMaxSelectionCount{
    if ([self.dataSource respondsToSelector:@selector(imagePickerControllerMaxSelectionCount:)]) {
        ...
        self.assetCollection.maxSelectionCount = 9;
...
-(void)finishPickingImages{
    [self.delegate imagePickerController:self didFinishPickingImagesList:[self.assetCollection getSelectedAssets]];
    [self dismissViewControllerAnimated:YES completion:nil];
}
