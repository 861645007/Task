...
- (void)initView{
    ...
    CGFloat offset = (1.0/32.0) * size.width;
    CGFloat imageWidth = self.bounds.size.width-offset;
    if (size.width > 320) {
        imageWidth -= 4;
    }
    
    // 选中时显示的图片
    imgview = [[UIImageView alloc]initWithFrame:CGRectMake((self.bounds.size.width - imageWidth) / 2, 10 + offset / 2, imageWidth, imageWidth)];
